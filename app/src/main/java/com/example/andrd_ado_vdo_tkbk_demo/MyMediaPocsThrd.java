...
//    private Message p_messagePt;
    ...
    TalkNetwork mTalkNetwork = new TalkNetwork();
    ...
    public void UserDstoy() {
            ...
            //如果本端TCP协议客户端套接字不为空或本端高级UDP协议套接字不为空且已连接远端。
        {
            OutExitPkt:
            {
                ...
                if (m_IsRecvExitPkt == 0) //如果没有接收到退出包。
                {
                    while (true) //循环接收退出包。
                    {
                        if (((mTalkNetwork.m_UseWhatXfrPrtcl == 0) && (mTalkNetwork.m_TcpClntSoktPt.RecvApkt(m_TmpBytePt, m_TmpBytePt.length, m_TmpHTLongPt, (short) 5000, 0, m_ErrInfoVstrPt) == 0)) ||
                                ((mTalkNetwork.m_UseWhatXfrPrtcl == 1) && (mTalkNetwork.m_AudpSoktPt.RecvApkt(mTalkNetwork.m_AudpCnctIdx.m_Val, m_TmpBytePt, m_TmpBytePt.length, m_TmpHTLongPt, (short) 5000, m_ErrInfoVstrPt) == 0))) {
                            if (m_TmpHTLongPt.m_Val != -1) //如果用本端套接字接收一个连接的远端套接字发送的数据包成功。
                            {
        ...
        //销毁本端TCP协议服务端套接字。
        if (mTalkNetwork.m_TcpSrvrSoktPt != null) {
            mTalkNetwork.m_TcpSrvrSoktPt.Dstoy(null); //关闭并销毁本端TCP协议服务端套接字。
        ...
        //销毁本端TCP协议客户端套接字。
        if (mTalkNetwork.m_TcpClntSoktPt != null) {
            mTalkNetwork.m_TcpClntSoktPt.Dstoy((short) -1, null); //关闭并销毁本端TCP协议客户端套接字。
        ...
        //销毁本端高级UDP协议套接字。
        if (mTalkNetwork.m_AudpSoktPt != null) {
            mTalkNetwork.m_AudpSoktPt.Dstoy(null); //关闭并销毁本端高级UDP协议套接字。
            mTalkNetwork.m_AudpSoktPt = null;
        ...
        if (m_RecvAdoOtptFrmLnkLstPt != null) {
            ...
            if (m_IsPrintLogcat != 0) Log.i(m_CurClsNameStrPt, "销毁接收音频输出帧链表成功。");
        }
        ...
        if (m_RecvVdoOtptFrmLnkLstPt != null) {
            ...
            if (m_IsPrintLogcat != 0) Log.i(m_CurClsNameStrPt, "销毁接收视频输出帧链表成功。");
        }
        ...
        if (mTalkNetwork.m_IsCreateSrvrOrClnt == 1) //如果是创建服务端。
        {
    ...
    //用户定义的读取音视频输入帧函数。
    @Override
    public void UserReadAdoVdoInptFrm(short PcmAdoInptSrcFrmPt[], short PcmAdoInptRsltFrmPt[], long PcmAdoInptFrmLenUnit, int PcmAdoInptRsltFrmVoiceActSts,
        ...
        if (PcmAdoInptSrcFrmPt != null) //如果有Pcm格式音频输入原始帧。
        {
            ...
            if (p_FrmPktLen != 1 + 4) //如果本音频输入帧为有语音活动，就发送。
            {
                ...
                if (((mTalkNetwork.m_UseWhatXfrPrtcl == 0) && (mTalkNetwork.m_TcpClntSoktPt.SendApkt(m_TmpBytePt, p_FrmPktLen, (short) 0, 1, 0, m_ErrInfoVstrPt) == 0)) ||
                        ((mTalkNetwork.m_UseWhatXfrPrtcl == 1) && (mTalkNetwork.m_AudpSoktPt.SendApkt(mTalkNetwork.m_AudpCnctIdx.m_Val, m_TmpBytePt, p_FrmPktLen, 1, m_ErrInfoVstrPt) == 0))) {
                    if (m_IsPrintLogcat != 0)
            ...
            } else //如果本音频输入帧为无语音活动。
            {
                if (m_LastSendAdoInptFrmIsAct != 0) //如果最后一个发送的音频输入帧为有语音活动，就发送。
                {
                    ...
                    if (((mTalkNetwork.m_UseWhatXfrPrtcl == 0) && (mTalkNetwork.m_TcpClntSoktPt.SendApkt(m_TmpBytePt, p_FrmPktLen, (short) 0, 1, 0, m_ErrInfoVstrPt) == 0)) ||
                            ((mTalkNetwork.m_UseWhatXfrPrtcl == 1) && (mTalkNetwork.m_AudpSoktPt.SendApkt(mTalkNetwork.m_AudpCnctIdx.m_Val, m_TmpBytePt, p_FrmPktLen, 10, m_ErrInfoVstrPt) == 0))) {
                        if (m_IsPrintLogcat != 0)
    ...
    //用户定义的获取音频输出帧函数。
    @Override
    public void UserGetAdoOtptFrm(int AdoOtptStrmIdx,
    ...
    //用户定义的获取视频输出帧函数。
    @Override
    public void UserGetVdoOtptFrm(int VdoOtptStrmIdx,
